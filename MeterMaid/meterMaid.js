...
async function main () {
  ...
  try {
    const myHoneywellMeterManager = new HoneywellMeterManager()
    const sessionID = await myHoneywellMeterManager.openConnection(
      config.honeywellUID,
      config.honeywellPass
    )

    console.log('sessionID: ', sessionID)
    let pollResults = await myHoneywellMeterManager.pollMeters(sessionID)
    console.log('pollResults: ', pollResults)
    myHoneywellMeterManager.closeConnection()
...
function checkIsOpenDuringPoll (site) {
  ...
  var dbPromise = new Promise(function (resolve, reject) {
    var checkOpenSQL = `SELECT * FROM LocationHours WHERE locationId = ${
